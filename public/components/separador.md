...
**Arquivo:** `base/components/separator.tsx` | **Categoria:** Conteúdo e dados | **Rota:** `/componentes/separador`
...
```tsx
import { Separator } from "@semec/base";
...
```text
Crie um separator (Separador) usando @semec/base (`base/components/separator.tsx`), estilo shadcn (Radix + CVA + clsx + tailwind-merge) em React + Tailwind CSS v4. Variantes: orientation: horizontal · vertical · decorative: true (role=none) · false (role=separator). Linha divisória visual. Horizontal ou vertical. Decorativo ou semântico.
```
