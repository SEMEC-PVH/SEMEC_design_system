---
name: semec-ds
description: Use SEMEC Design System (@semec/base) para construir UI. Tokens pv-*, componentes Radix+CVA, tema dark via data-theme. Use quando criar/editar UI, migrar portal, ou copiar snippets.
---

# SEMEC Design System — Skill para Claude

Kit `@semec/base` — React + Tailwind CSS v4 + shadcn (Radix + CVA + clsx + tailwind-merge). Conteúdo PT-BR, código EN (ADR-016).
...
## Instalação kit (base/README.md)

```bash
# Copie base/ para seu projeto (ex: src/base/)
...
Tokens trocam via `[data-theme="dark"]` em `base/tokens.css`. Proto isolado usa `PreviewFrame.jsx` para sync.
...
4. Valide variantes em `lib/base-manifest.js` (prop `variant`, `size` etc.)

Exemplo:
```tsx
import { Button, Card, CardHeader, CardTitle, CardContent, Badge } from "./base";
// ou arquivo: "./base/components/button"
...
- Prompt IA: `dsPrompt(c)` em `lib/base-manifest.js` → “Crie um button (Botões) usando @semec/base …”

## Protótipo isolado

`app/proto/proto.css` → `public/proto/proto.css` via `npm run proto:css` (`@source` varre `base/components` + `components/demos` + `components/docs`).
...
- Tokens: `base/tokens.css` + `base/shadcn.css` + `base/pv-preset.ts`
- Barrel: `base/index.ts`
